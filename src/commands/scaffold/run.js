...
(function() {
  ...
  module.exports = function(options, projectInfo) {
    ...
    if (!projectInfo) {
      ...
      global.edjcho.log('此操作会根据下面的问题，为efes项目在当前目录创建几个配置文件：');
      global.edjcho.log(chalk.green(chalk.bold('.eslintrc, .eslintignore')) + ': eslint检测规则和忽略规则，同时也是Sublime的插件Sublime-contrib-eslint配置文件');
      global.edjcho.log(chalk.green(chalk.bold('.csslintrc, .csslintignore')) + ': csslint检测规则和忽略规则');
      global.edjcho.log(chalk.green(chalk.bold('.efesconfig')) + ': efes项目配置文件');
      global.edjcho.log(chalk.green(chalk.bold('concatfile.json')) + ': efes项目文件合并配置规则');
      projectInfo = {};
